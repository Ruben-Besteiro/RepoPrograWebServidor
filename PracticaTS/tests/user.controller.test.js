...
describe('User Controller', () => {
    ...
    describe('refreshTokenCtrl', () => {
        it('debe revocar y emitir tokens', async () => {
            ...
            const storedToken = { token: 'oldToken', isActive: () => true, save: jest.fn(), user: { _id: 'u1' }, revoked: false };
    ...
    describe('updateUser', () => {
        it('actualiza datos', async () => {
            ...
            User.findById.mockReturnValue({ populate: jest.fn().mockResolvedValue(mockUpdateResult) });
    ...
    describe('restoreUser', () => {
        ...
        it('falla si ya está restaurado', async () => {
            req.body = { id: 'u1' };
            User.findById.mockResolvedValue({ deleted: false });
    ...
    describe('onboardUser', () => {
        it('falla si es freelance de otro', async () => {
            req.user = { _id: 'u1' };
            req.body = { company: 'c1' };
            Company.findById.mockResolvedValue({ _id: 'c1', isFreelance: true, owner: 'u2' });
        ...
        it('crea nueva compañía si no existe', async () => {
            ...
            req.body = {};
            Company.create.mockResolvedValue({ _id: 'new' });
            await onboardUser(req, res);
            expect(Company.create).toHaveBeenCalled();
            expect(res.status).toHaveBeenCalledWith(200);
        });
