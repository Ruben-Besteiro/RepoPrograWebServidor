import { jest, expect, it, beforeEach, describe } from '@jest/globals';
// Mocks de modelos
...
describe('Project Controller', () => {
    ...
    describe('createProject', () => {
        it('debería crear un proyecto si el cliente existe y no está archivado', async () => {
            ...
            Client.findById.mockResolvedValue({ _id: 'client123', deleted: false });
        ...
        it('debería lanzar error si el cliente está archivado', async () => {
            ...
            Client.findById.mockResolvedValue({ _id: 'client123', deleted: true });
    ...
    describe('updateProject', () => {
        it('debería actualizar un proyecto activo', async () => {
            ...
            const projectMock = { _id: 'p123', save: jest.fn().mockResolvedValue(true) };
            Project.findOneAndUpdate.mockResolvedValue(projectMock);
    ...
    describe('deleteProject', () => {
        it('debería marcar como borrado (soft delete)', async () => {
            req.params.id = 'p123';
            const saveMock = jest.fn().mockResolvedValue(true);
            Project.findById.mockResolvedValue({ _id: 'p123', save: saveMock });
    ...
    describe('getAllProjects', () => {
        it('debería retornar proyectos no borrados', async () => {
            Project.find.mockResolvedValue([{ name: 'P1' }]);
    ...
    describe('getProjectById', () => {
        it('debería retornar el proyecto si existe', async () => {
            req.params.id = 'p123';
            Project.findById.mockResolvedValue({ _id: 'p123', deleted: false });
        ...
        it('debería lanzar error si está archivado', async () => {
            req.params.id = 'p123';
            Project.findById.mockResolvedValue({ _id: 'p123', deleted: true });
    ...
    describe('archive/restore/listArchived', () => {
        it('debería archivar un proyecto', async () => {
            req.params.id = 'p1';
            const saveMock = jest.fn().mockResolvedValue(true);
            Project.findById.mockResolvedValue({ _id: 'p1', save: saveMock });
